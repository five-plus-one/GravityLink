...
def run():
    ...
    qr=c.ok('/api/v1/links','POST',{'type':'liveqr','entry_domain_id':entry['ID'],'landing_domain_id':landing['ID'],'landing_page_id':page['ID'],'title':'群码验收 '+stamp,'strategy':{'mode':'round_robin','targets':[{'target_url':'https://example.com/first.png','scan_limit':1},{'target_url':'https://example.com/second.png'}]}})
    ...
    status,_,html=visit(dest);check('first QR image rendered',status==200 and 'first.png' in html)
    status,_,html=visit(dest);check('threshold switches to next QR',status==200 and 'second.png' in html)
    ...
    status,_,html=visit(dest);check('expired and exhausted targets unavailable',status==200 and 'first.png' not in html and 'second.png' not in html and '暂无' in html)
    ...
    concurrent=c.ok('/api/v1/links','POST',{'type':'liveqr','entry_domain_id':entry['ID'],'landing_domain_id':landing['ID'],'landing_page_id':page['ID'],'title':'并发阈值验收 '+stamp,'strategy':{'mode':'round_robin','targets':[{'target_url':'https://example.com/limited.png','scan_limit':3},{'target_url':'https://example.com/fallback.png'}]}})
    concurrent_url=f"http://{landing['Host']}/{concurrent['Code']}"
    with ThreadPoolExecutor(max_workers=10) as pool:
        pages=list(pool.map(lambda _:visit(concurrent_url)[2],range(20)))
    check('concurrent threshold never over-allocates',sum('limited.png' in x for x in pages)==3 and sum('fallback.png' in x for x in pages)==17)
