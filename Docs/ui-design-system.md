...
- 导航断点统一为768px；手机菜单默认收起，选择页面后收起，隐藏菜单不接收交互。主内容只保留自身滚动区域，卡片与表格不撑宽外层网格。
- 链接桌面行将名称和访问地址合为主信息区，保留类型、目标、时间、状态和操作；手机使用链接卡片呈现复制、分享、统计、编辑和目标配置。
- 访客桌面表格限制滚动高度以保留分页；手机使用完整信息卡片并将分页放在列表顶部。
- 桌面使用双日历范围选择；手机独立选择开始和结束日期/时间，单日历内的确认按钮保持可见。
- 查询与统计具备加载、失败、空态和过期请求防覆盖，产品提示采用用户语言。
- 卡密项目详情串联库存、导入、领取链接与领取记录；提供名称选择及复制/二维码操作。

### 管理端页脚标识
管理端底部居中保留小型 GravityLink 标识：图标12px、文字10px、上下留白3px，链接使用 https://r-l.ink/glink，新窗口打开。
