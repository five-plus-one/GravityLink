...
卡密项目通过名称下拉选择。项目详情可选择入口域和落地域，生成绑定项目的领取页及活码链接；生成后提供地址复制、访问及入口二维码。创建链接失败时保留已创建领取页用于重试。

卡密导入按项目去重；项目锁覆盖去重插入，以及领取口令、重复规则与发放事务。同一项目同一访客并发领取遵守重复提取规则。库存与记录使用服务端分页，切换项目重置页码；导入、刷新和查看记录后更新库存数字。

公开页成功领取后保留本次卡密；复制成功后提示成功，复制失败提供手动操作提示。领取规则仍以访客 IP 识别同一访客，共用出口 IP 的用户受同一规则约束。
