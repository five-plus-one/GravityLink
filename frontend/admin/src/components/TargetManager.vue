...
<template>
 ...
 <NModal v-model:show="show" preset="card" title="活码二维码配置" style="width:min(1080px,96vw)" :mask-closable="false" :closable="!busy" :close-on-esc="!busy">
  <div class="toolbar">
   ...
   <NButton type="primary" :disabled="busy" @click="edit()">添加二维码</NButton>
   <BatchAdd :link-id="linkId" @saved="refresh"/>
   <MaterialPicker multiple relative :origin="origin" @select-many="addMaterials"/>
   <div class="batch-limit">
    <NInputNumber v-model:value="batchLimit" :min="1" :precision="0" size="small" clearable placeholder="统一阈值" style="width:130px"/>
    <NButton size="small" :loading="batchBusy" @click="applyBatchLimit">批量设阈值</NButton>
   </div>
  </div>
...
.batch-limit{display:flex;gap:6px;align-items:center;margin-left:auto}
table{width:100%;border-collapse:collapse;margin:20px 0}td,th{text-align:left;padding:12px 14px;border-bottom:1px solid #e6ecf5;white-space:nowrap}
